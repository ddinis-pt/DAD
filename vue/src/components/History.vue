...
onMounted(() => {
  ...
  document.title = 'Memory Card Game - History'
})
...
<template>
  <div class="min-h-screen bg-sky-50 dark:bg-gray-800">
      ...
      <div class="flex flex-col space-y-4">
        ...
        <div class="p-2 text-black dark:text-white">
          <nav class="-mb-0.5 flex flex-wrap gap-x-6 justify-center">
            <a class="py-4 px-1 inline-flex items-center gap-2 border-b-2 border-transparent text-sm whitespace-nowrap hs-tab-active:border  focus:border-blue-500 dark:focus:border-blue-500 hover:text-blue-500 dark:hover:text-blue-500 hover:cursor-pointer"
              @click.prevent="changeMode(1)"
              :class="{ 'text-blue-500 border-b-2 !border-blue-500': modeChosen === 'singleplayer' }">
              <i class="pi pi-user"></i>
              Singleplayer
            </a>
            <a class="py-4 px-1 inline-flex items-center gap-2 border-b-2 border-transparent text-sm whitespace-nowrap  focus:border-blue-500 dark:focus:border-blue-500 neutral-700 hover:text-blue-500 dark:hover:text-blue-500 hover:cursor-pointer"
              @click.prevent="changeMode(2)"
              :class="{ 'text-blue-500 border-b !border-blue-500': modeChosen === 'multiplayer' }" aria-current="page">
    ...
    <div v-if="modeChosen === 'singleplayer'" class="flex flex-grow justify-center">
      <DataTable v-model:filters="filters" :value="games" showGridLines filterDisplay="menu" paginator
        :loading="loading" removableSort :rows="5" :rowsPerPageOptions="[5, 10, 15]"
        ...
        <template #header>
          <div class="flex items-center justify-between">
            <div class="border rounded-lg border-blue-500">
              <Button style="color:" type="button" icon="pi pi-refresh" @click="refresh" text></Button>
            </div>


            <Button type="button" icon="pi pi-filter-slash" label="Clear" outlined @click="clearFilter"></Button>
          </div>
        </template>
        <Column sortable filterField="ended_at" field="ended_at" dataType="date" header="Date - Time">
          ...
          <template #filter="{ filterModel }">
            <DatePicker v-model="filterModel.value" dateFormat="dd/mm/yy" placeholder="dd/mm/yyyy"></DatePicker>
          </template>
        </Column>
        <Column field="board_id" header="Board" :showFilterMatchModes="false" :showFilterMenu="true">
          <template #body="{ data }">
          ...
          <template #filter="{ filterModel }">
            <MultiSelect v-model="filterModel.value" :options="boards" optionValue="value" optionLabel="label"
              placeholder="Any">
